...
class gap_analizer:
    ...
    def trafic_analyzer(self, input_video, model_path, linhas, output_csv_path, video_base_name):
        ...
        iniciofps = temporizador.calcular_segundos(frame_atual)
        print("inicio tempo por fps: ", iniciofps)
        # -------------------------------------------- OUTPUT PATHs --------------------------------------------------------------------
        ...
        while True:
            ...
            for result in resultTracker:
                ...
                cruzouAzul = (
                    ...
                    y1LinhaAzul <= cy <= y2LinhaAzul and
                    ((cx_anterior < x1LinhaAzul <= cx) or (cx_anterior > y1LinhaAzul >= cx))
                )
        ...
        if lista_entrada:
            print("lista_entrada não está vazia.")
        else:
            print("Lista vazia.")
    ...
    def executeGap(self, video_info):
        try:
            ...
            index = video_info.get("index")  # Pega o índice

            # Verifique se video_key não está vazio ou None
            if not video_path:
                raise ValueError("O caminho do vídeo não foi fornecido corretamente")
            # Usando diretamente o caminho local
            input_video = video_path  # Aqui você atribui diretamente o caminho do vídeo local

            #input_video = self.s3_service.download_video(video_key)
            video_base_name = os.path.splitext(os.path.basename(input_video))[0]  # Ex: "rotatoria"
        ...
        linhas = [
            # x1, y1, x2, y2, cor
            (260, 280, 350, 280, (0, 255, 0)),# horizontal
            (280,280,280,390,(255,0,0)), # vertical    
        ]
